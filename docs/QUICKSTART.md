...
The 5-minute path from `git clone` to a running ultraproject.

## 1. Prerequisites

- macOS or Linux, Python 3.9+, Node 18+
- Either [Claude Code](https://claude.com/claude-code) or [Codex CLI](https://github.com/openai/codex) installed and authenticated (or both — they're interchangeable)
- Optional: `ripgrep`, `gh`
...
This installs the `ultraproject` command on your PATH (the editable install means the repo source is what runs). The `pip` upgrade is required because pre-22.0 versions don't support editable installs from a `pyproject.toml` without a `setup.py`.
...
Copies `.env.example` → `.env`, `.mcp.example.json` → `.mcp.json`, and `vault/clients/_registry.example.md` → `vault/clients/_registry.md` if any are missing, then verifies that `claude` or `codex` is on your PATH.
...
## 4. Run An Ultraproject (Chat-Native)

Project execution is chat-native — there is no `ultraproject run` command. Open Claude Code or Codex pointed at this repo and paste:
...
Auth is whatever your CLI is configured for — Claude Code's subscription, Codex's, or your own API keys.
...
Token exhaustion, model switch, machine restart, or operator interruption — none of it loses progress. Just point a fresh Claude or Codex session at the repo with the same prompt, and it picks up from the last `ORCH-CHECKPOINT`.

The default `agent_mode` is `chat_native` — everything routes to whichever CLI is hosting the chat. If you're running cross-model (`agent_mode: normal`) and one CLI is exhausted, flip to a single-agent override:
