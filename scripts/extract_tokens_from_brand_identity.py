#!/usr/bin/env python3
"""Validate and normalize a manual brand_identity token JSON template."""
...
REQUIRED = ["logo_clear_space", "brand_color", "brand_type", "application_rules", "lockup_variants"]


def normalize_brand_payload(data: dict[str, Any]) -> dict[str, Any]:
    payload = data.get("tokens") if isinstance(data.get("tokens"), dict) else data
    missing = [name for name in REQUIRED if name not in payload]
    if missing:
        raise ValueError("manual brand token JSON is missing required families: " + ", ".join(missing))
    colors = payload["brand_color"]
    for key in ["primary", "secondary", "accent", "monochrome"]:
        value = colors.get(key, [])
        if isinstance(value, str):
            value = [value]
        colors[key] = [normalize_hex(v) or "#000000" for v in value]
    app = payload["application_rules"]
    contexts = app.get("contexts", [])
    if len(contexts) < 5:
        raise ValueError("application_rules.contexts must list at least 5 distinct application contexts")
    variants = payload["lockup_variants"].get("variants", [])
    if len(variants) < 2:
        raise ValueError("lockup_variants.variants must include at least 2 variants")
    return {name: payload[name] for name in REQUIRED}


def extract_tokens(mockup: Path) -> dict[str, Any]:
    if mockup.is_dir():
        candidates = sorted(mockup.rglob("*.json"))
        if not candidates:
            raise ValueError("brand_identity extraction requires a manual JSON template in the mockup directory")
        mockup = candidates[0]
    data = read_json(mockup)
    if not isinstance(data, dict):
        raise ValueError("manual brand token template must be a JSON object")
    return normalize_brand_payload(data)
