...
version: "1.0"
description: "Paged document visual contracts for reports, books, PDFs, and long-form typographic systems."
applicable_aesthetic_axes:
  density: [sparse, balanced, dense]
  topology: [single_panel, multi_region, chart_grid, narrative_sequence]
  expressiveness: [quiet, restrained, editorial, expressive]
  motion: [static]
  platform: [print]
  trust: [professional, enterprise, financial, clinical, luxury]
medium_extension_axes:
  format_register:
    description: "Document genre that determines reading rhythm, evidence density, and pagination discipline."
    values: [editorial, technical, report, book]
    required: true
mockup_format: page_mockup_pngs_plus_css_paged_media_or_tex_tokens
mockup_anchor_count_min: 3
mockup_revisions_per_anchor_min: 3
token_families: [type_scale, page_grid, margin_system, leading_rules, widow_orphan_rules, chart_styling, color_narrative]
token_extractor_script: scripts/extract_tokens_from_document_typography.py
parity_methodology:
  method: mixed
  renderer_script: scripts/preset_regression_check.py
  comparison_metrics: [page_ssim, type_scale_ast, margin_token_presence, widow_orphan_rules]
  thresholds:
    ssim_regen_min: 0.85
    type_scale_match_min_pct: 90
    margin_variance_max_pct: 3
  required_artifacts: [png, css, tex, manifest]
runtime_check_methodology:
  capture_method: deterministic_pdf_render_to_page_png
  checks: [page_ssim, pagination, type_scale, chart_style, widow_orphan]
  budgets:
    render_seconds: 120
    token_walk_seconds: 30
gospel_template_path: skills/templates/gospel-document_typography.md
applicable_presets: [editorial_premium, executive_analytics, data_scientific]
regression_replay_contract:
  supported: true
  renderer_script: scripts/preset_regression_check.py
  required_source_artifacts: [css, tex, png, manifest]
  supported_token_mutations: ["type_scale.*", "page_grid.*", "margin_system.*", "leading_rules.*", "widow_orphan_rules.*", "chart_styling.*", "color_narrative.*"]
  unsupported_token_mutations: []
  unsupported_mutation_behavior: skip
  replay_fixture: tests/fixtures/replay-document_typography/
  replay_determinism_test: tests/test_replay_deterministic_document_typography.py
---
# document_typography Medium Plugin

## Overview

Document Typography visual specifications cover PDF reports, whitepapers, books, annual reports, research documents, consulting decks exported as reports, and typographic systems with page evidence.

This medium excludes slide decks, web dashboards, standalone chart specs, and brand identity sheets where pagination is not the main contract.

The locked mockup format is `page_mockup_pngs_plus_css_paged_media_or_tex_tokens`. The minimum anchor set is 3 anchors: cover, body spread, chart spread.
...
The extension values are: editorial, technical, report, book.
...
For document_typography, token extraction covers: type_scale, page_grid, margin_system, leading_rules, widow_orphan_rules, chart_styling, color_narrative.
...
Create at least 3 anchor mockups. Required examples for this medium: cover, body spread, chart spread.
...
- `mockups/scaffold-1-{anchor}-rev1` plus the source extension for page_mockup_pngs_plus_css_paged_media_or_tex_tokens.
...
Run the extractor with: `python3 scripts/extract_tokens_from_document_typography.py --mockup PATH --out visual-references/tokens-document_typography.json`.

Accepted source inputs include: CSS Paged Media, LaTeX preamble, page PNGs, PDF render metadata.
The extractor reads CSS @page rules, margins, font declarations, line-height, color variables, and LaTeX geometry/font commands.
...
Parity for document_typography uses `mixed` comparison.
Primary acceptance threshold: Rendered page PNGs should match locked page mockups at SSIM 0.85 and preserve type scale, margins, and chart styling tokens.

The parity packet must include:
- Required artifacts: png, css, tex, manifest.
- Comparison metrics: page_ssim, type_scale_ast, margin_token_presence, widow_orphan_rules.
...
Runtime checks use `deterministic_pdf_render_to_page_png`.
Runtime evidence must map implementation outputs back to locked anchors.

Runtime checks enabled by this plugin: page_ssim, pagination, type_scale, chart_style, widow_orphan.
...
- Document Typography anchors that ignore `format_register`.
- Missing any token family from: type_scale, page_grid, margin_system, leading_rules, widow_orphan_rules, chart_styling, color_narrative.
...
The build-agent gospel template for this medium is `skills/templates/gospel-document_typography.md`.
...
`type_scale` controls type scale decisions for document_typography.
It must be extracted from the locked source artifact whenever CSS Paged Media or an equivalent structured format is available.
...
If `type_scale` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### page_grid

`page_grid` controls page grid decisions for document_typography.
It must be extracted from the locked source artifact whenever CSS Paged Media or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `page_grid` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### margin_system

`margin_system` controls margin system decisions for document_typography.
It must be extracted from the locked source artifact whenever CSS Paged Media or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `margin_system` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### leading_rules

`leading_rules` controls leading rules decisions for document_typography.
It must be extracted from the locked source artifact whenever CSS Paged Media or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `leading_rules` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### widow_orphan_rules

`widow_orphan_rules` controls widow orphan rules decisions for document_typography.
It must be extracted from the locked source artifact whenever CSS Paged Media or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `widow_orphan_rules` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### chart_styling

`chart_styling` controls chart styling decisions for document_typography.
It must be extracted from the locked source artifact whenever CSS Paged Media or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `chart_styling` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### color_narrative

`color_narrative` controls color narrative decisions for document_typography.
It must be extracted from the locked source artifact whenever CSS Paged Media or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `color_narrative` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.
...
For document_typography, preferred reference formats are: CSS Paged Media, LaTeX preamble, page PNGs, PDF render metadata.
...
Mockup format: `page_mockup_pngs_plus_css_paged_media_or_tex_tokens`.
...
1. Does every anchor prove the declared `format_register` value with concrete visual evidence?
2. Are all required token families present: type_scale, page_grid, margin_system, leading_rules, widow_orphan_rules, chart_styling, color_narrative?
...
- Defines the document_typography medium plugin frontmatter contract.
- Adds token extraction through `scripts/extract_tokens_from_document_typography.py`.
- Adds gospel template `skills/templates/gospel-document_typography.md`.
- Declares regression replay support as `CSS Paged Media or LaTeX deterministic re-render`.
