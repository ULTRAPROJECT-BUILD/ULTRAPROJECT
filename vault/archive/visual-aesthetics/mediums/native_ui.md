...
medium: native_ui
version: "1.0"
description: "Native mobile and desktop interface visual contracts for platform-idiomatic applications."
applicable_aesthetic_axes:
  density: [sparse, balanced, dense]
  topology: [single_panel, list_detail, multi_region]
  expressiveness: [quiet, restrained, expressive, playful]
  motion: [static, subtle, functional, expressive]
  platform: [ios_native, android_native, desktop_native, cross_platform]
  trust: [approachable, professional, enterprise, clinical, financial, safety_critical]
medium_extension_axes:
  platform_native:
    description: "Target platform idiom that the native surface must preserve."
    values: [ios_idiomatic, android_idiomatic, desktop_idiomatic, cross_platform_acceptable]
    required: true
mockup_format: figma_or_sketch_export_png_plus_token_json
mockup_anchor_count_min: 2
mockup_revisions_per_anchor_min: 3
token_families: [dynamic_type_scale, color, spacing, corner_radius, elevation, motion, safe_area_insets, focus, materials]
token_extractor_script: scripts/extract_tokens_from_native_ui.py
parity_methodology:
  method: mixed
  renderer_script: scripts/preset_regression_check.py
  comparison_metrics: [ssim, platform_idiom, token_json_ast, safe_area_geometry]
  thresholds:
    ssim_regen_min: 0.8
    token_presence_min_pct: 85
    safe_area_variance_max_pct: 5
  required_artifacts: [png, json, platform_manifest]
runtime_check_methodology:
  capture_method: simulator_or_desktop_screenshot_with_accessibility_tree
  checks: [ssim_runtime, platform_idiom, safe_area, dynamic_type, focus_visibility]
  budgets:
    capture_seconds: 45
    token_walk_seconds: 30
gospel_template_path: skills/templates/gospel-native_ui.md
applicable_presets: [operator_triage, operator_admin, apple_consumer, apple_native, things_calm]
regression_replay_contract:
  supported: true
  renderer_script: scripts/preset_regression_check.py
  required_source_artifacts: [figma_json, sketch_json, png]
  supported_token_mutations: ["dynamic_type_scale.*", "color.*", "spacing.*", "corner_radius.*", "elevation.*", "motion.*", "safe_area_insets.*", "focus.*", "materials.*"]
  unsupported_token_mutations: []
  unsupported_mutation_behavior: skip
  replay_fixture: tests/fixtures/replay-native_ui/
  replay_determinism_test: tests/test_replay_deterministic_native_ui.py
---
# native_ui Medium Plugin

## Overview

Native UI visual specifications cover iOS apps, Android apps, macOS utilities, Windows desktop tools, Electron/Tauri surfaces that intentionally preserve native conventions, and mobile prototypes that will become platform code.

This medium excludes browser-first apps, decks, standalone chart specifications, game HUDs, and pure marketing pages.

The locked mockup format is `figma_or_sketch_export_png_plus_token_json`. The minimum anchor set is 2 anchors: default screen, settings or list screen.
...
The extension values are: iOS-idiomatic, Android-idiomatic, desktop-idiomatic, cross-platform-acceptable.
...
For native_ui, token extraction covers: dynamic_type_scale, color, spacing, corner_radius, elevation, motion, safe_area_insets, focus, materials.
...
Create at least 2 anchor mockups. Required examples for this medium: default screen, settings or list screen.
...
- `mockups/scaffold-1-{anchor}-rev1` plus the source extension for figma_or_sketch_export_png_plus_token_json.
...
Run the extractor with: `python3 scripts/extract_tokens_from_native_ui.py --mockup PATH --out visual-references/tokens-native_ui.json`.

Accepted source inputs include: Figma plugin JSON, Sketch document JSON, platform screenshot PNGs, token JSON.
The extractor walks Figma or Sketch JSON, reads text styles, fills, effects, spacing, safe-area metadata, focus metadata, and material blur effects.
...
Parity for native_ui uses `mixed` comparison.
Primary acceptance threshold: Runtime screenshots should reach SSIM 0.80 or higher against the locked anchor, with platform idiom and safe-area checks passing.

The parity packet must include:
- Required artifacts: png, json, platform_manifest.
- Comparison metrics: ssim, platform_idiom, token_json_ast, safe_area_geometry.
...
Runtime checks use `simulator_or_desktop_screenshot_with_accessibility_tree`.
Runtime evidence must map implementation outputs back to locked anchors.

Runtime checks enabled by this plugin: ssim_runtime, platform_idiom, safe_area, dynamic_type, focus_visibility.
...
- Native UI anchors that ignore `platform_native`.
- Missing any token family from: dynamic_type_scale, color, spacing, corner_radius, elevation, motion, safe_area_insets, focus, materials.
...
The build-agent gospel template for this medium is `skills/templates/gospel-native_ui.md`.
...
`dynamic_type_scale` controls dynamic type scale decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
...
If `dynamic_type_scale` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### color

`color` controls color decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `color` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### spacing

`spacing` controls spacing decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `spacing` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### corner_radius

`corner_radius` controls corner radius decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `corner_radius` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### elevation

`elevation` controls elevation decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `elevation` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### motion

`motion` controls motion decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `motion` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### safe_area_insets

`safe_area_insets` controls safe area insets decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `safe_area_insets` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### focus

`focus` controls focus decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `focus` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### materials

`materials` controls materials decisions for native_ui.
It must be extracted from the locked source artifact whenever Figma plugin JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `materials` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.
...
For native_ui, preferred reference formats are: Figma plugin JSON, Sketch document JSON, platform screenshot PNGs, token JSON.
...
Mockup format: `figma_or_sketch_export_png_plus_token_json`.
...
1. Does every anchor prove the declared `platform_native` value with concrete visual evidence?
2. Are all required token families present: dynamic_type_scale, color, spacing, corner_radius, elevation, motion, safe_area_insets, focus, materials?
...
- Defines the native_ui medium plugin frontmatter contract.
- Adds token extraction through `scripts/extract_tokens_from_native_ui.py`.
- Adds gospel template `skills/templates/gospel-native_ui.md`.
- Declares regression replay support as `Figma-token-replay`.
