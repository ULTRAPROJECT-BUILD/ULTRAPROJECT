...
medium: presentation
version: "1.0"
description: "Slide deck visual contracts for staged narrative, executive communication, and product storytelling."
...
medium_extension_axes:
  narrative_arc:
    description: "Deck-level narrative structure that determines slide sequence and repeated visual motifs."
    values: [chronological, thematic, comparative, mixed]
    required: true
mockup_format: per_slide_png
mockup_anchor_count_min: 4
mockup_revisions_per_anchor_min: 3
token_families: [slide_grid, stage_spotlight, type_on_stage, slide_color, transition, build_pacing]
token_extractor_script: scripts/extract_tokens_from_presentation.py
parity_methodology:
  method: mixed
  renderer_script: scripts/preset_regression_check.py
  comparison_metrics: [per_slide_ssim, slide_grid_ast, type_token_presence, transition_map]
  thresholds:
    ssim_regen_min: 0.85
    grid_token_match_min_pct: 90
    slide_count_match: 1
  required_artifacts: [png, pptx, html, manifest]
runtime_check_methodology:
  capture_method: deck_renderer_per_slide_png
  checks: [per_slide_ssim, slide_order, type_presence, transition_presence]
  budgets:
    render_seconds: 90
    token_walk_seconds: 30
gospel_template_path: skills/templates/gospel-presentation.md
applicable_presets: [editorial_premium, executive_analytics, apple_consumer, vercel_marketing, playful_consumer, data_scientific]
regression_replay_contract:
  supported: true
  renderer_script: scripts/preset_regression_check.py
  required_source_artifacts: [slide_source, png, manifest]
  supported_token_mutations: ["slide_grid.*", "stage_spotlight.*", "type_on_stage.*", "slide_color.*", "transition.*", "build_pacing.*"]
  unsupported_token_mutations: []
  unsupported_mutation_behavior: skip
  replay_fixture: tests/fixtures/replay-presentation/
  replay_determinism_test: tests/test_replay_deterministic_presentation.py
---
# presentation Medium Plugin

## Overview

Presentation visual specifications cover PPTX decks, Keynote decks, Reveal.js decks, launch narratives, board updates, sales narratives, pitch decks, and analytical presentations.

This medium excludes long-form reports, standalone chart grammar, web pages, video timelines, and live product UI.

The locked mockup format is `per_slide_png`. The minimum anchor set is 4 anchors: title slide, content slide, section break, two representative slides.
...
The extension values are: chronological, thematic, comparative, mixed.
...
For presentation, token extraction covers: slide_grid, stage_spotlight, type_on_stage, slide_color, transition, build_pacing.
...
Create at least 4 anchor mockups. Required examples for this medium: title slide, content slide, section break, two representative slides.
...
- `mockups/scaffold-1-{anchor}-rev1` plus the source extension for per_slide_png.
...
Run the extractor with: `python3 scripts/extract_tokens_from_presentation.py --mockup PATH --out visual-references/tokens-presentation.json`.

Accepted source inputs include: PPTX slide-master XML, Reveal.js HTML, deck JSON, per-slide PNGs.
The extractor reads slide size, master layouts, theme colors, text runs, transition XML, Reveal sections, and timing hints.
...
Parity for presentation uses `mixed` comparison.
Primary acceptance threshold: Each locked slide should regenerate at SSIM 0.85 or higher and preserve the slide-grid and type-on-stage tokens.

The parity packet must include:
- Required artifacts: png, pptx, html, manifest.
- Comparison metrics: per_slide_ssim, slide_grid_ast, type_token_presence, transition_map.
...
Runtime checks use `deck_renderer_per_slide_png`.
Runtime evidence must map implementation outputs back to locked anchors.

Runtime checks enabled by this plugin: per_slide_ssim, slide_order, type_presence, transition_presence.
...
- Presentation anchors that ignore `narrative_arc`.
- Missing any token family from: slide_grid, stage_spotlight, type_on_stage, slide_color, transition, build_pacing.
- Fewer than 4 anchor mockups or fewer than three captured revisions per anchor.
...
The build-agent gospel template for this medium is `skills/templates/gospel-presentation.md`.
...
`slide_grid` controls slide grid decisions for presentation.
It must be extracted from the locked source artifact whenever PPTX slide-master XML or an equivalent structured format is available.
...
If `slide_grid` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### stage_spotlight

`stage_spotlight` controls stage spotlight decisions for presentation.
It must be extracted from the locked source artifact whenever PPTX slide-master XML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `stage_spotlight` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### type_on_stage

`type_on_stage` controls type on stage decisions for presentation.
It must be extracted from the locked source artifact whenever PPTX slide-master XML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `type_on_stage` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### slide_color

`slide_color` controls slide color decisions for presentation.
It must be extracted from the locked source artifact whenever PPTX slide-master XML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `slide_color` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### transition

`transition` controls transition decisions for presentation.
It must be extracted from the locked source artifact whenever PPTX slide-master XML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `transition` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### build_pacing

`build_pacing` controls build pacing decisions for presentation.
It must be extracted from the locked source artifact whenever PPTX slide-master XML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `build_pacing` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.
...
For presentation, preferred reference formats are: PPTX slide-master XML, Reveal.js HTML, deck JSON, per-slide PNGs.
...
1. Does every anchor prove the declared `narrative_arc` value with concrete visual evidence?
2. Are all required token families present: slide_grid, stage_spotlight, type_on_stage, slide_color, transition, build_pacing?
3. Are there at least 4 anchors and three captured revisions per anchor?
...
- Defines the presentation medium plugin frontmatter contract.
- Adds token extraction through `scripts/extract_tokens_from_presentation.py`.
- Adds gospel template `skills/templates/gospel-presentation.md`.
- Declares regression replay support as `Reveal.js + chromium-headless re-render`.
