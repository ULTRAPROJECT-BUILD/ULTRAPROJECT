...
version: "1.0"
description: "Logo system and application-study visual contracts for brand marks, lockups, and usage rules."
applicable_aesthetic_axes:
  density: [sparse, balanced]
  topology: [identity_system, lockup_family, application_suite]
  expressiveness: [restrained, editorial, expressive, playful]
  motion: [static]
  platform: [brand_system]
  trust: [approachable, professional, enterprise, financial, luxury]
medium_extension_axes:
  mark_personality:
    description: "Structural personality of the mark and lockup system."
    values: [geometric, organic, wordmark-only, symbol-and-wordmark, dynamic]
    required: true
mockup_format: application_study_pngs
mockup_anchor_count_min: 5
mockup_revisions_per_anchor_min: 3
token_families: [logo_clear_space, brand_color, brand_type, application_rules, lockup_variants]
token_extractor_script: scripts/extract_tokens_from_brand_identity.py
parity_methodology:
  method: application_suite
  renderer_script: scripts/preset_regression_check.py
  comparison_metrics: [application_context_count, lockup_integrity, small_size_survival, color_rule_presence]
  thresholds:
    application_context_count_min: 5
    lockup_integrity_min_pct: 95
    small_size_readability_min: 1
  required_artifacts: [png, svg, json, manifest]
runtime_check_methodology:
  capture_method: exported_application_studies_and_vector_inspection
  checks: [application_diversity, clear_space, minimum_size, monochrome_viability]
  budgets:
    review_seconds: 120
    token_walk_seconds: 30
gospel_template_path: skills/templates/gospel-brand_identity.md
applicable_presets: []
regression_replay_contract:
  supported: false
  renderer_script: scripts/preset_regression_check.py
  required_source_artifacts: [png, manual_json, svg]
  supported_token_mutations: []
  unsupported_token_mutations: ["*"]
  unsupported_mutation_behavior: operator_review
  replay_fixture: tests/fixtures/replay-brand_identity/
  replay_determinism_test: operator_review_required
---
# brand_identity Medium Plugin

## Overview

Brand Identity visual specifications cover Wordmarks, symbols, signature systems, logo usage sheets, social avatars, document mastheads, product headers, packaging studies, and brand application suites.

This medium excludes single marketing pages without an identity system, app UI without logo rules, and purely typographic document styling.

The locked mockup format is `application_study_pngs`. The minimum anchor set is 5 anchors: wordmark, signature, three application studies.
...
The extension values are: geometric, organic, wordmark-only, symbol-and-wordmark, dynamic.
...
For brand_identity, token extraction covers: logo_clear_space, brand_color, brand_type, application_rules, lockup_variants.
...
Create at least 5 anchor mockups. Required examples for this medium: wordmark, signature, three application studies.
...
- `mockups/scaffold-1-{anchor}-rev1` plus the source extension for application_study_pngs.
...
Run the extractor with: `python3 scripts/extract_tokens_from_brand_identity.py --mockup PATH --out visual-references/tokens-brand_identity.json`.

Accepted source inputs include: manual brand token JSON, SVG, application PNGs, PDF identity sheet.
The extractor reads a manual JSON template and validates that each required brand family is complete before writing payload tokens.
...
Parity for brand_identity uses `application_suite` comparison.
Primary acceptance threshold: At least five distinct application contexts must exist and the mark must preserve clear-space, minimum-size, and lockup rules.

The parity packet must include:
- Required artifacts: png, svg, json, manifest.
- Comparison metrics: application_context_count, lockup_integrity, small_size_survival, color_rule_presence.
...
Runtime checks use `exported_application_studies_and_vector_inspection`.
Runtime evidence must map implementation outputs back to locked anchors.

Runtime checks enabled by this plugin: application_diversity, clear_space, minimum_size, monochrome_viability.
...
- Brand Identity anchors that ignore `mark_personality`.
- Missing any token family from: logo_clear_space, brand_color, brand_type, application_rules, lockup_variants.
- Fewer than 5 anchor mockups or fewer than three captured revisions per anchor.
...
The build-agent gospel template for this medium is `skills/templates/gospel-brand_identity.md`.
...
`logo_clear_space` controls logo clear space decisions for brand_identity.
It must be extracted from the locked source artifact whenever manual brand token JSON or an equivalent structured format is available.
...
If `logo_clear_space` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### brand_color

`brand_color` controls brand color decisions for brand_identity.
It must be extracted from the locked source artifact whenever manual brand token JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `brand_color` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### brand_type

`brand_type` controls brand type decisions for brand_identity.
It must be extracted from the locked source artifact whenever manual brand token JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `brand_type` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### application_rules

`application_rules` controls application rules decisions for brand_identity.
It must be extracted from the locked source artifact whenever manual brand token JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `application_rules` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### lockup_variants

`lockup_variants` controls lockup variants decisions for brand_identity.
It must be extracted from the locked source artifact whenever manual brand token JSON or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `lockup_variants` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.
...
For brand_identity, preferred reference formats are: manual brand token JSON, SVG, application PNGs, PDF identity sheet.
...
Mockup format: `application_study_pngs`.
...
1. Does every anchor prove the declared `mark_personality` value with concrete visual evidence?
2. Are all required token families present: logo_clear_space, brand_color, brand_type, application_rules, lockup_variants?
3. Are there at least 5 anchors and three captured revisions per anchor?
...
- Defines the brand_identity medium plugin frontmatter contract.
- Adds token extraction through `scripts/extract_tokens_from_brand_identity.py`.
- Adds gospel template `skills/templates/gospel-brand_identity.md`.
- Declares regression replay support as `manual application studies`.
