...
medium: game_ui
version: "1.0"
description: "Game menu, HUD, overlay, and in-world interface visual contracts."
applicable_aesthetic_axes:
  density: [sparse, balanced, dense, ultra_dense]
  topology: [single_panel, multi_region, canvas]
  expressiveness: [restrained, expressive, playful, cinematic]
  motion: [static, subtle, functional, expressive, rapid]
  platform: [game_engine]
  trust: [experimental, approachable, professional]
medium_extension_axes:
  diegetic_axis:
    description: "Relationship between the UI and the game world."
    values: [in-world, meta, abstract-overlay]
    required: true
mockup_format: engine_export_pngs_or_external_mockup_pngs_plus_config_json
mockup_anchor_count_min: 2
mockup_revisions_per_anchor_min: 3
token_families: [hud_density, font_legibility_distance, color_state_encoding, animation_timing, audio_cue_vocabulary]
token_extractor_script: scripts/extract_tokens_from_game_ui.py
parity_methodology:
  method: mixed
  renderer_script: scripts/preset_regression_check.py
  comparison_metrics: [engine_screenshot_ssim, hud_geometry, legibility_distance, animation_timing]
  thresholds:
    ssim_runtime_min: 0.8
    legibility_distance_min: 1
    hud_token_presence_min_pct: 85
  required_artifacts: [png, json, engine_scene]
runtime_check_methodology:
  capture_method: engine_screenshot_across_representative_game_states
  checks: [hud_geometry, contrast_on_motion, input_focus, audio_cues, legibility_scale]
  budgets:
    capture_seconds: 120
    token_walk_seconds: 45
gospel_template_path: skills/templates/gospel-game_ui.md
applicable_presets: [playful_consumer]
regression_replay_contract:
  supported: false
  renderer_script: scripts/preset_regression_check.py
  required_source_artifacts: [engine_config, png, scene_file]
  supported_token_mutations: []
  unsupported_token_mutations: ["*"]
  unsupported_mutation_behavior: operator_review
  replay_fixture: tests/fixtures/replay-game_ui/
  replay_determinism_test: operator_review_required
---
# game_ui Medium Plugin

## Overview

Game UI visual specifications cover Game main menus, HUDs, inventory screens, pause menus, quest logs, cooldown bars, targeting overlays, in-world devices, and game-specific UI kits.

This medium excludes ordinary web apps, non-interactive video overlays, and data dashboards without gameplay context.

The locked mockup format is `engine_export_pngs_or_external_mockup_pngs_plus_config_json`. The minimum anchor set is 2 anchors: main menu, HUD.
...
The extension values are: in-world, meta, abstract-overlay.
...
For game_ui, token extraction covers: hud_density, font_legibility_distance, color_state_encoding, animation_timing, audio_cue_vocabulary.
...
Create at least 2 anchor mockups. Required examples for this medium: main menu, HUD.
...
- `mockups/scaffold-1-{anchor}-rev1` plus the source extension for engine_export_pngs_or_external_mockup_pngs_plus_config_json.
...
Run the extractor with: `python3 scripts/extract_tokens_from_game_ui.py --mockup PATH --out visual-references/tokens-game_ui.json`.

Accepted source inputs include: Unity prefab YAML, Unreal blueprint JSON export, Godot scene text, mockup CSS, engine config JSON.
The extractor reads HUD regions, font sizes, gameplay state colors, animation clips, audio cue names, and safe-zone geometry from engine files or CSS.
...
Parity for game_ui uses `mixed` comparison.
Primary acceptance threshold: Runtime engine screenshots should hit SSIM 0.80 and pass legibility-distance checks against representative gameplay backgrounds.

The parity packet must include:
- Required artifacts: png, json, engine_scene.
- Comparison metrics: engine_screenshot_ssim, hud_geometry, legibility_distance, animation_timing.
...
Runtime checks use `engine_screenshot_across_representative_game_states`.
Runtime evidence must map implementation outputs back to locked anchors.

Runtime checks enabled by this plugin: hud_geometry, contrast_on_motion, input_focus, audio_cues, legibility_scale.
...
- Game UI anchors that ignore `diegetic_axis`.
- Missing any token family from: hud_density, font_legibility_distance, color_state_encoding, animation_timing, audio_cue_vocabulary.
...
The build-agent gospel template for this medium is `skills/templates/gospel-game_ui.md`.
...
`hud_density` controls hud density decisions for game_ui.
It must be extracted from the locked source artifact whenever Unity prefab YAML or an equivalent structured format is available.
...
If `hud_density` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### font_legibility_distance

`font_legibility_distance` controls font legibility distance decisions for game_ui.
It must be extracted from the locked source artifact whenever Unity prefab YAML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `font_legibility_distance` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### color_state_encoding

`color_state_encoding` controls color state encoding decisions for game_ui.
It must be extracted from the locked source artifact whenever Unity prefab YAML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `color_state_encoding` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### animation_timing

`animation_timing` controls animation timing decisions for game_ui.
It must be extracted from the locked source artifact whenever Unity prefab YAML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `animation_timing` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.

### audio_cue_vocabulary

`audio_cue_vocabulary` controls audio cue vocabulary decisions for game_ui.
It must be extracted from the locked source artifact whenever Unity prefab YAML or an equivalent structured format is available.
The family should include named values rather than anonymous measurements whenever the source format provides names.
The build agent must use these values instead of tool defaults.
The runtime gate may inspect source, rendered evidence, and manifest tokens to confirm the family survived implementation.
If `audio_cue_vocabulary` is missing, the Visual Specification is incomplete for this medium.
Example evidence includes a source token, rendered measurement, reviewer note, and manifest JSON entry.
...
For game_ui, preferred reference formats are: Unity prefab YAML, Unreal blueprint JSON export, Godot scene text, mockup CSS, engine config JSON.
...
Mockup format: `engine_export_pngs_or_external_mockup_pngs_plus_config_json`.
...
1. Does every anchor prove the declared `diegetic_axis` value with concrete visual evidence?
2. Are all required token families present: hud_density, font_legibility_distance, color_state_encoding, animation_timing, audio_cue_vocabulary?
...
- Defines the game_ui medium plugin frontmatter contract.
- Adds token extraction through `scripts/extract_tokens_from_game_ui.py`.
- Adds gospel template `skills/templates/gospel-game_ui.md`.
- Declares regression replay support as `engine-dependent runtime scene`.
