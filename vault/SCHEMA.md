...
design_mode: stitch_required | concept_required | implementation_only | "" (optional — default: "". Canonical UI design contract when `ui_work: true`. `stitch_required` = explicit opt-in to Stitch MCP as the visual source of truth. `concept_required` = default for user-facing design work that must define a concept/visual direction without requiring Stitch. `implementation_only` = low-risk follow-through on an already-approved design/source of truth.)
stitch_required: true | false (optional — default: false. Backward-compatible mechanical flag for explicitly Stitch-governed work. Set true only when `design_mode: stitch_required` or when the operator/project explicitly requires Stitch MCP as the design source of truth.)
