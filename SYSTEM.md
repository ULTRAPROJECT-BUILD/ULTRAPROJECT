...
**Walkthrough video capture** — use `python scripts/ensure_qc_walkthrough.py ...` as the first-choice QC helper for browser/web and desktop/native flows. It infers when walkthrough video is required, reuses existing artifacts when present, and falls back to `capture_walkthrough_video.py` under the hood. For interactive browser/native deliverables, QC-stage walkthrough video is now part of the expected evidence surface, not just an optional extra. Review packs promote `.mp4/.mov/.webm` walkthrough artifacts alongside screenshots, and the polish gate can now require them when the artifact type is interactive.

**Artifact polish review** — clean-room review layer for the finished artifact pack after QC. This is domain-agnostic: websites, apps, decks, reports, data deliverables, and media all need a reviewer to ask whether the thing actually feels finished and credible. Use `python scripts/build_review_pack.py` to build the review pack, run [[artifact-polish-review]], then enforce it with `python scripts/check_polish_gate.py`.

**Google Stitch MCP** (`mcp__stitch__*`) — optional advanced UI design tool for high-fidelity screens and downloadable HTML/screenshot assets. Use it only when the operator, project plan, or ticket explicitly opts into Stitch as the visual source of truth. When enabled, start by finding or creating a Stitch project, then use `generate_screen_from_text` or `edit_screens`, and hand off finalized screens to the `react:components` skill. Available tools: `list_projects`, `get_project`, `create_project`, `list_screens`, `get_screen`, `generate_screen_from_text`, `generate_variants`, `edit_screens`. The `stitch-design` skill is the preferred entry point when Stitch is deliberately enabled.
For frontend design work, concept is not optional, but Stitch is optional. Tickets must carry `ui_work: true`, a `design_mode`, and tag `ui-design`. Use `design_mode: concept_required` by default for public surfaces, redesigns, rejected visual work, and other UI that needs a real concept, visual quality bar, and screenshot-driven QC. Use `design_mode: stitch_required` only when the operator explicitly asks for Stitch or a configured project intentionally makes Stitch the visual source of truth; those tickets also carry `stitch_required: true` and tag `stitch-required`. Use `design_mode: implementation_only` only for low-risk polish or faithful follow-through on an already-approved design/source of truth. Public-facing first-impression surfaces should also carry `public_surface: true` and must define a `Visual Quality Bar` plus `Narrative Structure`. Top-level nav/settings surfaces should carry `page_contract_required: true` and must define `Page Contracts`; destructive actions belong inside a danger zone, not as the whole page.
