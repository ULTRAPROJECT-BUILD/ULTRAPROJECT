=== VISUAL SPECIFICATION CONTRACT (3d_render) ===
...
Medium extension axis: realism={realism}
...
TOKEN FAMILIES:
- `key_light_direction` must be implemented from manifest tokens.
- `fill_ratio` must be implemented from manifest tokens.
- `rim_strength` must be implemented from manifest tokens.
- `ambient_color` must be implemented from manifest tokens.
- `material_roughness` must be implemented from manifest tokens.
- `material_specular` must be implemented from manifest tokens.
- `post_treatment` must be implemented from manifest tokens.
...
- If realism is set, preserve its medium-specific idiom in every runtime artifact.
...
- Preserve `key_light_direction` values exactly unless a Visual Specification amendment changes them.
- Preserve `fill_ratio` values exactly unless a Visual Specification amendment changes them.
- Preserve `rim_strength` values exactly unless a Visual Specification amendment changes them.
- Preserve `ambient_color` values exactly unless a Visual Specification amendment changes them.
- Preserve `material_roughness` values exactly unless a Visual Specification amendment changes them.
- Preserve `material_specular` values exactly unless a Visual Specification amendment changes them.
- Preserve `post_treatment` values exactly unless a Visual Specification amendment changes them.
