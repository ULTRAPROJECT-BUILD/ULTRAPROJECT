=== VISUAL SPECIFICATION CONTRACT (game_ui) ===
...
Medium extension axis: diegetic_axis={diegetic_axis}
...
TOKEN FAMILIES:
- `hud_density` must be implemented from manifest tokens.
- `font_legibility_distance` must be implemented from manifest tokens.
- `color_state_encoding` must be implemented from manifest tokens.
- `animation_timing` must be implemented from manifest tokens.
- `audio_cue_vocabulary` must be implemented from manifest tokens.
...
- If diegetic_axis is set, preserve its medium-specific idiom in every runtime artifact.
...
- Preserve `hud_density` values exactly unless a Visual Specification amendment changes them.
- Preserve `font_legibility_distance` values exactly unless a Visual Specification amendment changes them.
- Preserve `color_state_encoding` values exactly unless a Visual Specification amendment changes them.
- Preserve `animation_timing` values exactly unless a Visual Specification amendment changes them.
- Preserve `audio_cue_vocabulary` values exactly unless a Visual Specification amendment changes them.
