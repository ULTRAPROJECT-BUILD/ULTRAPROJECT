=== VISUAL SPECIFICATION CONTRACT (brand_identity) ===
...
Medium extension axis: mark_personality={mark_personality}
...
TOKEN FAMILIES:
- `logo_clear_space` must be implemented from manifest tokens.
- `brand_color` must be implemented from manifest tokens.
- `brand_type` must be implemented from manifest tokens.
- `application_rules` must be implemented from manifest tokens.
- `lockup_variants` must be implemented from manifest tokens.
...
- If mark_personality is set, preserve its medium-specific idiom in every runtime artifact.
...
- Preserve `logo_clear_space` values exactly unless a Visual Specification amendment changes them.
- Preserve `brand_color` values exactly unless a Visual Specification amendment changes them.
- Preserve `brand_type` values exactly unless a Visual Specification amendment changes them.
- Preserve `application_rules` values exactly unless a Visual Specification amendment changes them.
- Preserve `lockup_variants` values exactly unless a Visual Specification amendment changes them.
